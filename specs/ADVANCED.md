...
**Status:** To be implemented after Tasks 6 & 7
...
1. **Should `base.json` default to `Bundler` or `NodeNext`?**
   - **Decision Made:** `Bundler` for everything (simpler)
   - Backend explicitly opts into `NodeNext` when needed
   - Validated across 12 packages ✅

2. **What is blocking `yarn link` equivalent in pnpm?**
   - To be answered in Task 6

3. **What's the deployment strategy with frozen dependencies?**
   - To be answered in Task 7

4. **Next-gen linters (Biome, oxlint) - when to adopt?**
   - Current recommendation: Stick with ESLint for now
   - Strategic preference: oxlint (Vite alignment) when production-ready
   - Monitor adoption: Wait for 10%+ adoption before switching
...
### Primary Reference: HeroUI (Perfect Match)
